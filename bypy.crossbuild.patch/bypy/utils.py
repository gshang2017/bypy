...
#
    if os.path.exists('/opt/armv7l-linux-musleabihf-cross'):
        defs = {
            ...
            'CMAKE_C_COMPILER': '/opt/armv7l-linux-musleabihf-cross/bin/armv7l-linux-musleabihf-gcc',
            'CMAKE_CXX_COMPILER': '/opt/armv7l-linux-musleabihf-cross/bin/armv7l-linux-musleabihf-g++',
        }
    else:
        defs = {
            'CMAKE_BUILD_TYPE': 'RELEASE',
            ...
            'CMAKE_SYSTEM_NAME': 'Linux',
            'CMAKE_C_COMPILER': '/opt/aarch64-linux-musl-cross/bin/aarch64-linux-musl-gcc',
            'CMAKE_CXX_COMPILER': '/opt/aarch64-linux-musl-cross/bin/aarch64-linux-musl-g++',
        }
    if ismacos:
        defs.update({
...
#
    if use_cross:
        if os.path.exists('/opt/armv7l-linux-musleabihf-cross'):
            cmd += shlex.split('--cross-file /bypy/patches/armv7.txt')
        else:
            cmd += shlex.split('--cross-file /bypy/patches/arm64.txt')
    if extra_cmdline:
