...
from bypy.constants import (CFLAGS, LDFLAGS, MAKEOPTS, NMAKE, PERL, build_dir,
                            is64bit, ismacos, iswindows, islinux, current_build_arch)
from bypy.utils import run, replace_in_file, ModifiedEnv
...
#
        if not os.path.exists('/opt/armv7l-linux-musleabihf-cross'):
...
#
        run('./config', '--prefix=/usr', '--cross-compile-prefix=', '--libdir=lib',
            '--openssldir=/etc/ssl', 'shared', 'no-asm', 'no-tests', 'zlib', '-Wa,--noexecstack',
